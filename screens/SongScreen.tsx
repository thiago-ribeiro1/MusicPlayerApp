...
import TrackPlayer, { useActiveTrack, usePlaybackState, State, useProgress, RepeatMode, Track } from 'react-native-track-player';
...
const SongScreen = () => {
  ...
  if (shuffleMode) {
    const remainingSongs = songs.filter(song => song.url !== currentTrack?.url);
    const next = remainingSongs[Math.floor(Math.random() * remainingSongs.length)];
...
const handleSkipToPrevious = useCallback(async () => {
  ...
  if (shuffleMode) {
    const remainingSongs = songs.filter(song => song.url !== currentTrack?.url);
    const prev = remainingSongs[Math.floor(Math.random() * remainingSongs.length)];
            ...
            <View style={tw`flex-1 items-center justify-start mt-28`}>
              ...
              <View style={tw`flex-row items-end h-16 w-11/12 justify-center`}>
                {waveform.map((value, index) => {
                  const isPlayed = index < ((progress.position || 0) / (progress.duration || 1)) * waveform.length;
                  return (
                    <Pressable
                      key={index}
